    ...
    public function getListDriver(Request $request) {
        if($request->isMethod('GET')) {
            ...
            $driverList = Driver::join('all_global_param', 'ex_master_driver.driver_status', 'all_global_param.id')
                          ...
                          ->select('ex_master_driver.*', 'all_global_param.param_name as status_name', 'ex_master_kenek.kenek_name', 'usr_detail.no_rek', 'usr_detail.nama_bank', 'usr_detail.nama_rekening', 'usr_detail.nomor_hp')
                          ->orderBy('id', 'ASC')
                          ->get();
