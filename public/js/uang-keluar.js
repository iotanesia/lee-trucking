$("document").ready(function(){
  ...
  $.ajax({
    url: window.Laravel.app_url + "/api/moneyTransactionHeader/get-list-uang-keluar",
    type: "GET",
    dataType: "json",
  ...
  $("#moneyTransactionHeader-modal").on("show.bs.modal", function(e) {
    ...
    if(invoker.attr('el-event') == 'edit') {
      ...
      $("#moneyTransactionHeader-modal #btn-submit").hide();
      ...
      bindToForm($("#moneyTransactionHeader-modal"), dataJSON);
        ...
        $("#moneyTransactionHeader-modal #btn-submit").show();
...
var successLoadmoneyTransactionHeader = (function(responses, dataModel) {
  ...
  for(var i = 0; i < responses.data.length; i++) {
    ...
    date = responses.data[i].date;
    category_name = responses.data[i].category_name;
    ...
    tableRows += "<tr>" +
                   "<td>"+ (i+1) +"</td>"+
                   "<td>"+ def(category_name) +"</td>"+
                   "<td>"+ convertToRupiah(pokok) +"</td>"+
                   "<td>"+ dateFormat(date) +"</td>"+
                   "<td align='left'>"+
                   "<a class='btn btn-warning btn-xs btn-sm' href='#' el-event='edit' data-json='"+ data_json +"' data-toggle='modal' data-target='#moneyTransactionHeader-modal'><i class='fa fa-eye'></i></a>"+
                   "</td>"+
                //    "<td align='left'><a class='btn btn-warning btn-sm' href='"+window.Laravel.app_url+"/pinjaman-karyawan/detail/"+id+"'>Detail</a></td>"+
                 "</tr>";
  }
